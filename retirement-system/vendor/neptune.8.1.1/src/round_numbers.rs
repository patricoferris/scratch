...
pub(crate) fn round_numbers_strengthened(arity: usize) -> (usize, usize) {
    ...
    let strengthened_partial_rounds = partial_rounds * 5 / 4 as usize;
    ...
    for mut rf_test in (2..=1000).step_by(2) {
        for mut rp_test in 4..200 {
            if round_numbers_are_secure(t, rf_test, rp_test) {
                if security_margin {
                    rf_test += 2;
                    // TODO: Check diff with float + ceil
                    rp_test = 43 * rp_test / 40;
                }
...
fn round_numbers_are_secure(t: usize, rf: usize, rp: usize) -> bool {
    ...
    let rf_interp = ((430 * m) / 100) + (t.ilog2() as usize) - rp;
    let rf_grob_1 = ((210 * n) / 100) - rp;
    let rf_grob_2 = (((140 * n)/100) - 1 - rp) / (t - 1);
    let rfs = [rf_stat, rf_interp, rf_grob_1, rf_grob_2];
    let rf_max = rfs
        .iter()
